...
namespace Mediapipe.Unity.Sample.HandTracking
{
  public class HandTrackingSolution : ImageSourceSolution<HandTrackingGraph>
  {
    ...
    protected override IEnumerator WaitForNextValue()
    {
      ...
      if (result.handLandmarks != null && result.handedness != null && result.handLandmarks.Count == result.handedness.Count)
      {
          for (int i = 0; i < result.handLandmarks.Count; i++)
          {
              var landmarksList = result.handLandmarks[i];
              var handednessInfo = result.handedness[i];
              if (handednessInfo.Classification.Count > 0)
              {
                  var handedness = handednessInfo.Classification[0].Label;

                  if (handedness == "Right")
                  {
                      HandGesture.Instance.DetectGestures(landmarksList);
                  }
              }
          }
      }
